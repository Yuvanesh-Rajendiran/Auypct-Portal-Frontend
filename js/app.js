...
function submitForm() {
    ...
    .then(data => {
        ...
        alertDiv.innerHTML = `
            <div id="successBanner" style="position: fixed; top: 20px; left: 50%; transform: translateX(-50%); background: #48bb78; color: white; padding: 15px 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); z-index: 1000; display: flex; align-items: center; gap: 10px; animation: slideIn 0.5s ease; transition: opacity 0.5s ease;">
                <span>✅ Application Submitted Successfully!</span>
                <span>Tracking ID: ${data.trackingId}</span>
                <button onclick="navigator.clipboard.writeText('${data.trackingId}'); this.textContent='Copied!'; setTimeout(() => { this.textContent='Copy ID'; fadeOutBanner(); }, 1000);" style="background: rgba(255,255,255,0.2); border: none; color: white; padding: 5px 10px; border-radius: 5px; cursor: pointer;">Copy ID</button>
                <button onclick="fadeOutBanner()" style="background: transparent; border: none; color: white; font-size: 18px; cursor: pointer;">❌</button>
            </div>
            <style>
                @keyframes slideIn {
...
function fadeOutBanner() {
    ...
    if (banner) {
        ...
        setTimeout(() => banner.remove(), 500); // remove after fade
    }
}
