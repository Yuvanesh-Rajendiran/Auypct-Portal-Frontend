...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (token && (userRole === 'admin' || userRole === 'trustee')) {
    showNavbar();
    showDashboard();
  } else {
    hideNavbar();
...
function login() {
  ...
  fetch(API_CONFIG.login, {
    ...
    body: JSON.stringify({ username, password, role: selectedRole }) // include role in request
  })
    .then(res => res.json())
    .then(data => {
      // ✅ Check if returned role matches selected role
      if (data.token && data.role === selectedRole) {
        // Save login state
        ...
        setTimeout(() => {
          ...
          showNavbar(); // ✅ show navbar after login
          showDashboard(); // redirect after login
        }, 2000);
        ...
        button.innerHTML = originalText;
        button.disabled = false;
        validationList.innerHTML = `<li>${data.error || 'Access denied: Role mismatch or not authorized'}</li>`;
        validationSummary.style.display = 'block';
        hideNavbar(); // keep navbar hidden on failure
      }
    })
    .catch(err => {
...
async function updateStatus(trackingId, status) {
  try {
    ...
    const reviewer = localStorage.getItem('role') === 'admin' ? 'Admin' : 'Trustee';
    const res = await fetch(`${API_CONFIG.statusUpdate}/${trackingId}/status`, {
      ...
      body: JSON.stringify({ status: formattedStatus, reviewedBy: reviewer })
    });
...
async function addBank(trackingId) {
  ...
  try {
    ...
    const res = await fetch(`${API_CONFIG.statusUpdate}/${trackingId}/status`, {
      ...
      body: JSON.stringify({ status: 'Funds Transferred', bankDetails: { details: bankDetails }, reviewedBy: reviewer })
    });
    const data = await res.json();
    if (data.success) {
...
function logout() {
  localStorage.removeItem('token');
  localStorage.removeItem('role');
  hideNavbar(); // ✅ hide navbar after logout
  location.reload();
}
